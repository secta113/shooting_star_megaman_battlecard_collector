...
def main():
    ...
    for c in cards["standard"]:
        ...
        ch_id = chapter_map.get(ch_num, "ch_ox")
    ...
    for ch in chapters:
        ...
        title_js = json.dumps(ch["title"], ensure_ascii=False)
        desc_js = json.dumps(ch["desc"], ensure_ascii=False)
        flowchart_areas_js += "  {\n"
...
"""
    ...
    # 5. Write ryusei1_cards_routing.md
    md = []
    md.append("# 流星のロックマン1 カード獲得ストーリーフローチャート")
    md.append("本チャートは、ゲームのストーリー進行度（第1章〜クリア後）に応じて、**どのカードが最も早く手に入るか**を分類・整理したものです。")
    ...
    output_md_file = os.path.join(script_dir, "ryusei1_cards_routing.md")
